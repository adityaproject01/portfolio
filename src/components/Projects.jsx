      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((proj, idx) => (
            <div key={idx} className="glass rounded-2xl p-6 hover-glow">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {proj.tech.map((t, i) => (
                  <span key={i} className="px-3 py-1 bg-opacity-20 rounded-full text-sm" style={{ backgroundColor: 'rgba(255,255,255,0.1)' }}>{t}</span>
                ))}
              </div>
              <button className={`w-full py-2 ${proj.border} rounded-lg hover:bg-opacity-20 transition-all`}>
                View Project
              </button>
            </div>
          ))}
